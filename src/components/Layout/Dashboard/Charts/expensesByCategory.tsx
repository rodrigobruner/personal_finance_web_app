...
export default function ExpensesByCategory(props: ExpensesByCategoryProps) {
    ...
    const fetchAll = async (): Promise<void> => {
        try {
            const response = await axios.get(`http://localhost:8080/Transactions/category-summary/Expense/${formState.start.value.format('YYYY-MM-DD')}/${formState.end.value.format('YYYY-MM-DD')}`);
