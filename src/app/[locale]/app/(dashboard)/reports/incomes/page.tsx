...
export default function IncomeListPage(
    ...
    const t = useMemo(() => (messages as any).Pages.IncomesReport, [messages]);
    ...
    const fetchAll = async (): Promise<void> => {
        try {
            ...
            if (!session) return;
            //Get the incomes from the API
            const response = await axios.get(`http://localhost:8080/Transactions/user/${session?.uid}/category/Income`);
    ...
    const handleAddButton = useCallback(() => {
        ...
        router?.push(`/${locale}/app/reports/incomes/form/0`);
    }, [router, locale]);

    // Handle edit account
    const handleEditButton = useCallback((id: number) => {
        // Redirect to the edit account page
        router?.push(`/${locale}/app/reports/incomes/form/${id}`);
    }, [router, locale]);
